...
def fully_connected(inputs,
    ...
    mu_initializer = tf.initializers.random_uniform(minval=-stddev, maxval=stddev)
    sigma_initializer = tf.constant_initializer(value=constant)

    with tf.variable_scope(scope):
        mu_w = tf.get_variable('mu_w', weight_shape, trainable=True, initializer=mu_initializer)
        sigma_w = tf.get_variable('sigma_w',
                                  weight_shape,
                                  trainable=True,
                                  initializer=sigma_initializer)
        mu_b = tf.get_variable('mu_b', biases_shape, trainable=True, initializer=mu_initializer)
...
class NoisyDQNAgent(base_dqn_agent.DQNAgent):
    ...
    """
        ...
        super(NoisyDQNAgent, self).__init__(sess=sess,
