...
def masked_conv2d(inputs,
  ...
  """
    ...
    mask_list = [
        np.zeros(tuple(kernel_size) + (num_inputs, num_outputs // output_multiplier),
    ...
    with tf.compat.v1.variable_scope(scope):
        w = tf.compat.v1.get_variable('W', kernel_shape, trainable=True, initializer=weights_initializer)
        b = tf.compat.v1.get_variable('biases', biases_shape, trainable=True, initializer=biases_initializer)
...
def gating_layer(x, embedding, hidden_units, scope_name=''):
    ...
    with tf.compat.v1.variable_scope(scope_name):
        out = masked_conv2d(x,
                            ...
                            output_multiplier=2,
                            scope='masked_conv')
        out += slim.conv2d(embedding, 2 * hidden_units, [1, 1], activation_fn=None)
...
class CTSIntrinsicReward(object):
    ...
    """
        ...
        if (len(observation_shape) != 2 or observation_shape[0] != observation_shape[1]):
            raise ValueError('Observation shape needs to be square')
        self._observation_shape = observation_shape
        self.density_model = shannon.CTSTensorModel(observation_shape, convolutional)
...
class PixelCNNIntrinsicReward(object):
    ...
    def __init__(self,
        ...
        with tf.device(tf_device), tf.name_scope('intrinsic_pixelcnn'):
            ...
            self.obs_ph = tf.compat.v1.placeholder(tf.uint8,
                                                   shape=observation_shape,
                                                   name='obs_ph')
            self.preproccessed_obs = self._preprocess(self.obs_ph, resize_shape)
            self.iter_ph = tf.compat.v1.placeholder(tf.uint32, shape=[], name='iter_num')
            self.eval_ph = tf.compat.v1.placeholder(tf.bool, shape=[], name='eval_mode')
            self.network = tf.compat.v1.make_template('PixelCNN', self._network_template)
            self.ipd = tf.cond(tf.logical_not(self.eval_ph), self.update, self.virtual_update)
    ...
    """
        ...
        return self._sess.run(self.reward, {
    ...
    def _preprocess(self, obs, obs_shape):
        ...
        obs = tf.compat.v1.image.resize_bilinear(obs, obs_shape)
        denom = tf.constant(256 // self.quantization_factor, dtype=tf.float32)
        return tf.compat.v1.floordiv(obs, denom)
    ...
    def _network_template(self, obs, num_layers, hidden_units):
        ...
        with slim.arg_scope(
            [slim.conv2d, masked_conv2d],
                weights_initializer=tf.initializers.variance_scaling(distribution='uniform'),
            ...
            for i in range(1, num_layers + 1):
                net2 = gating_layer(net, embedding, hidden_units, 'gating_{}'.format(i))
            ...
            net += slim.conv2d(embedding, hidden_units, [1, 1], activation_fn=None)
            net = tf.nn.relu(net)
            ...
            logits = masked_conv2d(net,
                                   ...
                                   mask_type='B',
                                   activation_fn=None)
        loss = tf.compat.v1.losses.sparse_softmax_cross_entropy(labels=tf.cast(obs, tf.int32),
                                                      logits=logits,
                                                      reduction=tf.compat.v1.losses.Reduction.MEAN)
        return collections.namedtuple('PixelCNN_network', ['logits', 'loss'])(logits, loss)
    ...
    def update(self):
        ...
        with tf.name_scope('update'):
            ...
            with tf.name_scope('post_update'), tf.control_dependencies([train_op]):
                ...
                ipd = (loss - loss_post_training) * (self.resize_shape[0] * self.resize_shape[1])
        return ipd
    ...
    def virtual_update(self):
        ...
        with tf.name_scope('virtual_update'):
            ...
            backup_op = tf.group(*[t.assign(s) for t, s in zip(saved_vars, model_vars)])
            ...
            with tf.control_dependencies([loss_post_training]):
                restore_op = tf.group(*[d.assign(s) for d, s in zip(model_vars, saved_vars)])
    ...
    def ipd_to_reward(self, ipd, steps):
        ...
        ipd = self.ipd_scale * ipd / tf.sqrt(tf.compat.v1.to_float(steps))
        inv_pseudo_count = tf.maximum(tf.math.expm1(ipd), 0.0)
...
class RNDIntrinsicReward(object):
    ...
    def __init__(self,
                 ...
                 tf_device='/gpu:0',
                 reward_scale=1.0,
                 optimizer=tf.keras.optimizers.Adam(learning_rate=0.0001, epsilon=0.00001),
                 summary_writer=None):
        ...
        with tf.device(tf_device), tf.name_scope('intrinsic_rnd'):
            ...
            self.iter_ph = tf.compat.v1.placeholder(tf.uint64, shape=[], name='iter_num')
            self.iter = tf.cast(self.iter_ph, tf.float32)
            self.obs_ph = tf.compat.v1.placeholder(tf.uint8, shape=obs_shape, name='obs_ph')
            self.eval_ph = tf.compat.v1.placeholder(tf.bool, shape=[], name='eval_mode')
    ...
    def _target_network(self, obs):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
                            trainable=False,
                            weights_initializer=tf.keras.initializers.Orthogonal(gain=np.sqrt(2)),
                            biases_initializer=tf.zeros_initializer()):
            net = slim.conv2d(obs, 32, [8, 8], stride=4, activation_fn=tf.nn.leaky_relu)
            net = slim.conv2d(net, 64, [4, 4], stride=2, activation_fn=tf.nn.leaky_relu)
            net = slim.conv2d(net, 64, [3, 3], stride=1, activation_fn=tf.nn.leaky_relu)
            net = slim.flatten(net)
            embedding = slim.fully_connected(net, self.embedding_size, activation_fn=None)
        return embedding
    ...
    def _update_moments(self):
        ...
        def update():
            ...
            moments = [(self.obs, self.obs_mean, self.obs_std),
    ...
    def _build_train_op(self):
        ...
        self.loss = tf.compat.v1.losses.mean_squared_error(
            target, prediction, reduction=tf.compat.v1.losses.Reduction.MEAN)
