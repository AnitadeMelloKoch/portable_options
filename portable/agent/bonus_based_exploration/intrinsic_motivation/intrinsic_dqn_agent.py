...
class PixelCNNDQNAgent(base_intrinsic_agent.IntrinsicDQNAgent):
    ...
    """
        ...
        super(PixelCNNDQNAgent, self).__init__(sess=sess,
...
class RNDDQNAgent(base_intrinsic_agent.IntrinsicDQNAgent):
    ...
    """
        ...
        self.intrinsic_model = intrinsic_rewards.RNDIntrinsicReward(sess=sess,
                                                                    tf_device=tf_device,
                                                                    summary_writer=summary_writer)
        super(RNDDQNAgent, self).__init__(sess=sess,
                                          num_actions=num_actions,
                                          observation_shape=observation_shape,
                                          gamma=gamma,
                                          update_horizon=update_horizon,
    ...
    def _add_intrinsic_reward(self, observation, extrinsic_reward):
        ...
        if self._clip_reward:
            np.clip(intrinsic_reward, -1., 1., out=intrinsic_reward)
            np.clip(reward, -1., 1., out=reward)
        if (self.summary_writer is not None
                and self.training_steps % self.summary_writing_frequency == 0):
            summary = tf.Summary(value=[
                tf.Summary.Value(tag='Train/ExtrinsicReward', simple_value=extrinsic_reward),
                tf.Summary.Value(tag='Train/IntrinsicReward', simple_value=intrinsic_reward),
                tf.Summary.Value(tag='Train/TotalReward', simple_value=reward)
            ])
