...
from portable.agent.bonus_based_exploration import TrainRunnerForOptions

from portable.agent.bonus_based_exploration.intrinsic_motivation import intrinsic_dqn_agent
from portable.agent.bonus_based_exploration.intrinsic_motivation import intrinsic_rainbow_agent
from portable.agent.bonus_based_exploration.noisy_networks import noisy_dqn_agent
from portable.agent.bonus_based_exploration.noisy_networks import noisy_rainbow_agent
from portable.agent.bonus_based_exploration.intrinsic_motivation import intrinsic_rainbow_agent_for_options
from dopamine.discrete_domains import run_experiment
import gin


@gin.configurable
def create_exploration_agent(sess, environment, action_num=None, agent_name=None,
    ...
    if action_num is None:
        action_num = environment.action_space.n
    if not debug_mode:
        summary_writer = None
    if agent_name == 'dqn_cts':
        return intrinsic_dqn_agent.CTSDQNAgent(
            sess, num_actions=action_num,
    ...
    if agent_name == 'dqn_pixelcnn':
        ...
        return noisy_rainbow_agent.NoisyRainbowAgent(
            sess, num_actions=action_num,
            summary_writer=summary_writer)
    elif agent_name == 'rainbow_rnd_skill':
        return intrinsic_rainbow_agent_for_options.RNDRainbowAgentForSkills(
            sess, num_actions=action_num,
            summary_writer=summary_writer
        )
    else:
...
def create_exploration_runner(base_dir, create_agent_fn,
    ...
    elif schedule == 'option_agent':
        return TrainRunnerForOptions(base_dir, create_agent_fn)
    else:
