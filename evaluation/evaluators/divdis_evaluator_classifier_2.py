...
class DivDisEvaluatorClassifier():
    ...
    def __init__(
        ...
        for i in range(self.head_num):
            self.integrated_gradients.append(NoiseTunnel(IntegratedGradients(
                self.classifier.classifier.model[i])
            ))
    ...
    def _evaluate_images(self, images):
        ...
        for image in images:
            vote_list = []
            single_image = image.unsqueeze(0)
            labels_pred = self.classifier.predict(single_image).detach() # (batch_size, head_num, num_classes)
            labels_pred = torch.argmax(labels_pred, dim=2)
