...
positive_train_files = [img_dir + "climb_up_ladder_room1_termination_positive.npy"]
negative_train_files = [
    img_dir + "climb_up_ladder_room1_termination_negative.npy",
...
if __name__ == "__main__":
        ...
        # calculate avg and std of weighted accuracy, raw accuracy
        weighted_accs = [history['weighted_accuracy'] for history in room_histories]
        raw_accs = [history['raw_accuracy'] for history in room_histories]

        avg_weighted_acc = np.mean(weighted_accs, axis=0)
        std_weighted_acc = np.std(weighted_accs, axis=0)
        avg_raw_acc = np.mean(raw_accs, axis=0)
        std_raw_acc = np.std(raw_accs, axis=0)

        print(f"Weighted Accuracy: {avg_weighted_acc}, {std_weighted_acc}")
        print(f"Raw Accuracy: {avg_raw_acc}, {std_raw_acc}")
