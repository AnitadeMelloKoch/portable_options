...
@gin.configurable
class DivDisClassifierSweepExperiment():
    def __init__(self,
                 base_dir,
                 experiment_name,
                 use_gpu,
        ...
        self.dataset_positive = UnbalancedSetDataset(max_size=1e6,
                                                batchsize=64)
        
        self.dataset_negative = UnbalancedSetDataset(max_size=1e6,
                                                batchsize=64)
        
        self.dataset_positive.set_transform_function(transform)
        self.dataset_negative.set_transform_function(transform)
        self.dataset_positive.set_load_data_function(load_image)
        self.dataset_negative.set_load_data_function(load_image)
    ...
    def sweep_class_div_weight(self,
        ...
        for weight in tqdm(weights, desc="weights", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                ...
                classifier = self.create_classifier(diversity_weight=weight)
    ...
    def sweep_ensemble_size(self,
        ...
        if ensemble_sizes is None:
            ensemble_sizes = range(start_size, end_size+1, step_size)

        
        for num_heads in tqdm(ensemble_sizes, desc="size", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                set_seed(self.seed+seed)
                classifier = self.create_classifier(head_num=num_heads)
    ...
    def sweep_div_batch_size(self,
        ...
        for batch_size in tqdm(batch_sizes, desc="batch_sizes", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                set_seed(self.seed+seed)
                classifier = self.create_classifier(div_batch_size=batch_size)
    ...
    def sweep_lr(self,
        ...
        for lr in tqdm(lrs, desc="lrs", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                set_seed(self.seed+seed)
                classifier = self.create_classifier(learning_rate=lr)
    ...
    def sweep_l2_reg_weight(self,
        ...
        for reg_weight in tqdm(l2_weights, desc="l2_reg_weights", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                set_seed(self.seed+seed)
                classifier = self.create_classifier(l2_reg_weight=reg_weight)
    ...
    def sweep_class_weights(self,
        ...
        weights = np.linspace(start_ratio, end_ratio, int(num_samples), endpoint=True)

        for weight in tqdm(weights, desc="class_weights", position=0):
            class_weights = [weight, 1 - weight]
            results_weights.append(class_weights)
            weight_acc = []
            weight_avg_acc = []
            weight_loss = []
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                ...
                classifier.dataset.class_weight = class_weights
        ...
        save_dict = {"class_weights": results_weights,
                     "accuracies": results_acc,
                     "avg_accuracies": results_avg_acc,
                     "losses": results_loss,}
        self.save_results_dict(save_dict,
                               "class_weights_sweep.pkl")

        self.plot(os.path.join(self.plot_dir, "class_weights_sweep.png"),
                  weights,
                  results_acc,
                  results_avg_acc,
                  results_loss,
                  "Class Weights",
                  "Class Weights (Class 0)",
                  log_scale=False)
    ...
    def sweep_div_overlap(self,
        ...
        for overlap in tqdm(np.linspace(start_ratio, end_ratio, step_size), desc="unlabelled_overlap_ratio", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                ...
                classifier = self.create_classifier(unlabelled_files=unlabelled_files)
    ...
    def sweep_div_variety(self,
        ...
        for variety in tqdm(range(len(all_combination_files)), desc="unlabelled_variety_combinations", position=0):
            ...
            for seed in tqdm(range(num_seeds), desc="seeds", position=1, leave=False):
                set_seed(self.seed+seed)
                classifier = self.create_classifier(unlabelled_files=all_combination_files[variety])
    ...
    def create_classifier(self, 
        ...
        """Creates and configures a new classifier with optional parameters.

        Args:
            diversity_weight: Optional diversity weight, uses default if None
            head_num: Optional number of heads, uses default if None
            learning_rate: Optional learning rate, uses default if None
            l2_reg_weight: Optional L2 regularization weight, uses default if None
            
            positive_files: Optional positive training files, uses default if None
            negative_files: Optional negative training files, uses default if None
            unlabelled_files: Optional unlabelled files, uses default if None

        Returns:
            Configured DivDisClassifier instance
        """
        classifier = DivDisClassifier(
            use_gpu=self.use_gpu,
            log_dir=self.log_dir,
            diversity_weight=diversity_weight if diversity_weight is not None else self.default_div_weight,
            head_num=head_num if head_num is not None else self.default_num_heads,
            learning_rate=learning_rate if learning_rate is not None else self.default_lr,
            l2_reg_weight=l2_reg_weight if l2_reg_weight is not None else self.default_l2_reg_weight,
            unlabelled_dataset_batchsize=div_batch_size)

        classifier.dataset.set_transform_function(transform)
        classifier.dataset.set_load_data_function(load_image)

        classifier.add_data(
            positive_files=positive_files if positive_files is not None else self.train_positive_files,
            negative_files=negative_files if negative_files is not None else self.train_negative_files,
            unlabelled_files=unlabelled_files if unlabelled_files is not None else self.unlabelled_files
        )
        classifier.set_class_weights()
